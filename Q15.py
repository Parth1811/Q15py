...
class Q15no:
    ...
    def twos_compliment(self):
        ...
        self.hex_rep = self.format_hex(hex(2 * TWO_POW_15 - num))

    def format_hex(self, hex_val):
        regex = re.match('^(0x|0X)?([a-fA-F0-9]{1,4})$', hex_val)
        if regex == None:
            raise ArithmeticError("Please provide hex value in format '0x1a21'\
                                    and should not exceed 4 characters (i.e. 16bits)")
    ...
    def value(self):
        num = int(self.hex_rep, 16)
        if num >= TWO_POW_15:
            return -1 * float(2 * TWO_POW_15 - num) / TWO_POW_15
        else:
    ...
    def __mul__(self, b):
        ...
        if b.is_negative():
            b1.twos_compliment()
            flip = not flip
        result = Q15no(hex_val= hex((a1.int_val * b1.int_val) / TWO_POW_15))
        del a1, b1
        if flip:
